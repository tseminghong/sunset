...
const sendMessage = async (message: string): Promise<string> => {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorPayload.error || 'Unexpected response from tutor service')
    }

    const data = await response.json()
    return data.reply ?? 'Tutor paused. Try again.'
  } catch (error) {
...
export default function AIChat() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'model', text: 'Need help revising ICT? Ask HPCSS Bot anything. 💡' },
  ])
  ...
  return (
    <div className="fixed bottom-4 right-4 md:bottom-6 md:right-6 z-50 flex flex-col items-end pointer-events-auto cursor-auto">
      <AnimatePresence>
        {isOpen && (
          <motion.div
          ...
          >
            <div className="bg-gradient-to-r from-fuchsia-900/50 to-purple-900/50 p-4 flex justify-between items-center border-b border-white/10">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-heading font-bold text-white tracking-wider">HPCSS Bot</h3>
              </div>
            ...
            <div ref={chatContainerRef} className="h-64 md:h-80 overflow-y-auto p-4 space-y-3">
            ...
            <div className="p-3 border-t border-white/10 bg-black/40">
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="Ask about SQL, HTML..."
      ...
      <motion.button
        ...
        className="w-12 h-12 md:w-14 md:h-14 rounded-full bg-gradient-to-tr from-fuchsia-600 to-cyan-600 flex items-center justify-center shadow-lg shadow-fuchsia-500/40 border border-white/20"
        data-hover="true"
      >
        {isOpen ? <X className="w-5 h-5 md:w-6 md:h-6 text-white" /> : <MessageCircle className="w-5 h-5 md:w-6 md:h-6 text-white" />}
      </motion.button>
    </div>
  )
}
